...
def copy_selected_folders(source_directory, target_directory, selected_folders, root_directory):
    ...
    if not root_folder_path:
        return {"message": f"Root directory '{root_directory}' not found."}
    ...
    for folder in selected_folders:
        ...
        if folder_path:
            try:
                ...
                results.append({"message": f"Copied folder: '{folder_path}' to '{target_folder_path}'"})
            except Exception as e:
                results.append({"message": f"Failed to copy '{folder}': {e}"})
        else:
            results.append({"message": f"Folder '{folder}' does not exist in the root directory '{root_directory}'."})
